...
1. 如果没有文件被提交，退出并返回错误码和错误信息
2. 索引时，创建所有必备的树状文件
3. 创建提交对象，指向 root 树对象
4. 更新当前分支，指向提交记录
5. 清空索引



树的构建需要两步完成。首先，索引被转换为代表文件树的哈希结构。其次，此结构将转换为文件系统上的树对象。这两个步骤，通过递归完成。



对于提交信息，我们使用用户的 $VISUAL 编辑器环境变量。用户退出编辑器后，我们读取文件，将内容放入提交中。



让我们一起看看这一切。暂存并提交 `bin/rgit` 和`bin/rgit-add` 可在 `.rgit` 中返回以下结果：
...
为了找到当前状态，我们首先通过检查 `.rgit/HEAD`，来找到我们的分支。这指向 `.rgits/refs/heads/master`，即 `master` 分支。`master` 分支指向最后的提交记录。提交反过来指向代表项目根目录的树对象。这个树对象，指向另一个代表 `bin/` 目录的树对象，该对象又指向两个 `blob` 对象，这些对象包含在提交时 `bin/rgit` 和 `bin/rgit-add` 的压缩内容。



![img](https://images.thoughtbot.com/rebuilding-git-in-ruby/mTnEMJCNS2wM3a9JtbCw_rgit-commit-tree.png)
...
彼此对象之间互相指向这种结构，使 git 如此强大。通过简单地更改一些这些指向文件，我们可以切换到历史记录中的不同节点。
