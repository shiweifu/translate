...
Htmx提供了一种方法，可以轻松地将平滑的CSS转换附加到AJAX事件中，通常也可以在你的网页中。
...
htmx class-tool是一个扩展，您可以轻松地使用它来切换、添加或删除一个特定的类名，而无需编写任何 JavaScript 代码。



您可以通过向元素添加classes属性，然后指定操作，然后指定要添加或删除的类名，来利用这个扩展：
...
像上面的代码样例一样，一旦浏览器内容被加载，htmx将自动在1秒后的div中添加一个新类(sample-class)。
