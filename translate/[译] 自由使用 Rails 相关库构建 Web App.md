...
我们可以使用纯文本文件，但必须实现自己的格式以纯文本表示数据。 好消息是Ruby拥有一个内置库PStore，该库将Ruby对象转换为二进制文件并将其存储在文件中。 另外，PStore还能够从文件读取并将二进制文件转换回Ruby对象。 序列化和反序列化是将Ruby对象编组为二进制，反之亦然的过程。



但是，PStore将文件存储为人类无法读取的二进制文件。 因此，我们将上移并使用YAML :: Store，它是PStore的实现。 YAML :: Store不是将Ruby对象序列化为二进制，而是序列化为人类可读的YAML文件格式。
...
### 通过代码创建基于 YAML 的持久化数据



在 GitHub 上查看完整代码：[`mirth-4.rb `on GitHub](https://gist.github.com/ShopifyEng/c09cf3ce7cc506a778c5b46cc36a3aa9)。



1. 确保有 `mirth-4.rb` 和 YAML 文件，都在同一文件夹下。
2. 执行 `ruby mirth-4.rb`，在你的浏览器访问 `localhost:1337/show/birthdays`，你将看到和之前执行，返回的数据一致。
3. 输入一些数据，然后重启你的应用。你会发现先前输入的数据仍然在！YAML 文件的内容更新为刚刚输入的数据。



### 节点，你现在已经可以使用 Web 应用程序啦！



这很重要，如果哪个主题，你想特别了解的话，请参阅文章末尾的 “其他信息” 部分。



但请等一等，到此为止，我们编写的 Web 应用，全部只使用 Ruby 库。我们做了很多工作。我们需要了解的全部底层细节，TCP 如何运行，如何处理 HTTP 请求和进行响应，如何持久化数据，将其保存到文件。我的意思是还有更好的方式来编写 Ruby Web 应用。我们可以通过更高级别的抽象，来使编写 Web App 更加容易。
...
在所有类型的Web应用程序中，实现TCP套接字以创建网络连接的代码通常都是相似的-服务器创建一个套接字以接受客户端。 HTTP请求和响应也在所有类型的Web应用程序中标准化。 如上所述，重写该功能以解析HTTP需要花费大量时间和精力。 Web开发人员必须了解有关网络协议的复杂细节。



有机会将这些功能从试图制作Web应用程序的开发人员那里抽象出来，从而减少了工作量。 我们可以使用应用程序服务器来代替每次我们要启动新的Web应用程序时都创建网络连接并编写HTTP解析逻辑的方法。