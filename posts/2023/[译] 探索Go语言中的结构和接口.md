...
- 内嵌进结构体的字段不能用作结构中的字段名

- 字段声明后面可以跟一个可选的字符串文字标记

- 导出的结构字段必须以大写字母开头

- 除了基本类型之外，我们还可以将函数类型和接口类型作为结构字段



关于结构类型的更多详细信息可以在语言规范中[找到](https://golang.org/ref/spec#Struct_types)。
...
- 空接口包含零个方法。请注意，所有类型都实现了空接口。这意味着，如果编写一个以空接口｛｝值为参数的函数，则可以为该函数提供任何值

- 接口通常属于使用接口类型值的包，而不是实现这些值的包



关于接口类型的更多详细信息可以在语言规范中 [找到](http://https:https//golang.org/ref/spec#Interface_types)。
...
正如我们所了解到的，接口类型可以存储值的副本，或者可以通过存储指向值地址的指针来与接口共享值。关于接口类型需要注意的一点是，建议不要过早地关注优化，因为我们不想在使用接口之前定义接口。
