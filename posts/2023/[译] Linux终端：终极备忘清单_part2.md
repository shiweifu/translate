...
向 @bogkonstantin 和 @samuelabreu 大声致敬，因为他们让我知道了这些我不知道的命令。
...
上一篇文章中，我们看到通过 `history` 命令，来获得完整的历史命令列表。如果你不想看到完整的命令列表，但想快速的搜索某一条命令，键入`[ctrl] + r`。您将被要求输入几个字符以获得匹配的命令。您可以通过再次键入[CTRL]+r在这些结果之间进行循环查找。
...
- 打印文件的最后n行➜ `tail -n [行数] [文件名]`

- 打印文件的第一行➜  `head [文件名]`

- 打印文件的前n行➜ `head-n [行数] [文件名]`
